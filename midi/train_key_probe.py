...
class ActivationDataset(Dataset):
    ...
    def __init__(
        ...
        for i, meta in enumerate(self.metadata):
            ...
            # Flatten all dims except last (hidden_dim), then mean pool
            hidden_dim = layer_act.shape[-1]
            layer_act_flat = layer_act.reshape(-1, hidden_dim)  # [num_tokens, hidden_dim]
...
def main():
    ...
    LAYER_NAME = None  # or None to auto-pick last layer
    BATCH_SIZE = 32
    NUM_EPOCHS = 50
    LEARNING_RATE = 0.001
    TEST_SIZE = 0.2
    ...
    # === TOGGLE THIS ===
    MAJOR_MINOR_ONLY = False  # True = 2 classes (major/minor), False = 24 classes (all keys)
