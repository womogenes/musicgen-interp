...
# ---------------------------------------------------------------------
NUM_CHUNKS = 10
REMOTE_SPEC = "orcd-batch:/home/mjwei/orcd/pool"

# ---------------------------------------------------------------------
# RSYNC HELPER
# ---------------------------------------------------------------------
def rsync_batch(local_dir: Path):
    """
    rsyncs contents of local_dir to ORCD using ssh multiplexed session.
    Requires: `ssh orcd-batch` was run manually to open the master connection.
    """
    src = str(local_dir.resolve()) + "/"
    dest = REMOTE_SPEC

    cmd = ["rsync", "-avz", src, dest]
    print("\n[RSYNC] Running:", " ".join(cmd))
...
def detect_key_from_waveform_torch(wave, sr):
    ...
    if np.all(chroma_energy == 0): return "Unknown", 0
    ...
    for i,note in enumerate(NOTE_NAMES):
        ...
        if ms>best_score: best_score=ms; best_name=f"{note} major"
        if ns>best_score: best_score=ns; best_name=f"{note} minor"
...
def main():
    ...
    for mood in moods:
        for key_str in all_keys:
            ...
            style_prompt = get_style_prompt(muq, prompt=f"Classical, {mood}, Piano, key: {key_str}")
            ...
            for i in range(NUM_CHUNKS):
                ...
                # RSYNC entire batch directory
                rsync_batch(output_root)
